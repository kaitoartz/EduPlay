# 🃏 Game Catalog Cards & 3D Tilt Component

This document details the catalog cards improvements, including a ready-to-integrate 3D tilt component, demo structure, and additional micro-interactions.

---

## 🛠️ Card Enhancements

### 1. Hover 3D Tilt Effect
* Dynamically adjust rotation on the X and Y axes (`rotateX`, `rotateY`) based on cursor movement over the card (max 15 degrees).
* Implement a speculative highlight/shine element that follows the cursor as an overlay gradient.

### 2. Shimmer & Skeleton Loaders
* When loading games, display card skeleton elements matching the 16px corner radii, using a linear shimmer gradient animation.

### 3. Animated Badges
* Badges like "Nuevo", "Trending", or "Early Access" should transition into view with subtle entry animations.

### 4. Expandable Footer
* On hovering the card, the bottom area should expand downward smoothly, revealing brief descriptions, statistics, or custom tag groups.

---

## 💻 React 3D Card Component Code
...
### Integration Steps
1. **Directory Setup**: Copy the code above to `/components/ui/animated-3d-card.tsx` (or adjust import aliases `@/lib/utils` if necessary).
2. **Icons & Assets**: Ensure `lucide-react` is correctly imported. Map image cards to high-quality Unsplash URLs or local image assets.
3. **Responsive Config**: Adjust cards column distribution using standard tailwind classes or grid presets inside the parent component.
