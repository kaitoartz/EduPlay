...
const PremiumGameCard = ({ id, title, subject, level, duration, points, color, bg, icon: Icon, image, description, locked, tag, onClick, isDark }) => {
  ...
  return (
    <div
      ...
      className={`relative w-full rounded-[2rem] p-1 cursor-pointer group bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-150 dark:to-zinc-50 border border-zinc-200 dark:border-zinc-200 shadow-sm hover:shadow-lg transition-all duration-300 ${locked ? 'opacity-85' : ''}`}
    >
      ...
      <div className="relative bg-white dark:bg-zinc-50 rounded-[1.8rem] h-full flex flex-col z-10 overflow-hidden">
        <CardContentLayout
          id={id}
          title={title}
          subject={subject}
