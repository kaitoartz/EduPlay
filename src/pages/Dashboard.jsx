  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-6 lg:px-8 bg-zinc-50/50 dark:bg-zinc-950 transition-colors duration-300">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white dark:bg-zinc-50 p-10 rounded-[2rem] border border-zinc-100 dark:border-zinc-200 shadow-sm relative overflow-hidden transition-colors duration-300">
          ...
          <div className="flex items-center gap-8 relative z-10">
            <div className="relative group">
              <img src={user.avatar} alt="Avatar" className="w-28 h-28 rounded-full bg-orange-100 border-4 border-white dark:border-zinc-200 shadow-xl group-hover:scale-105 transition-transform" />
              <div className="absolute -bottom-2 -right-2 bg-gradient-to-r from-blue-600 to-cyan-500 text-white text-sm font-bold px-4 py-1.5 rounded-full border-2 border-white dark:border-zinc-150 shadow-lg">Lvl {user.level}</div>
            </div>
            <div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white dark:bg-zinc-50 p-10 rounded-[2rem] border border-zinc-100 dark:border-zinc-200 shadow-sm transition-colors duration-300">
              <h3 className="text-2xl font-bold text-zinc-900 dark:text-white mb-8">Progreso de Nivel</h3>
              <div className="relative h-8 bg-zinc-100 dark:bg-zinc-150 rounded-full overflow-hidden mb-4 shadow-inner">
              ...
              <div className="flex justify-between items-center mb-8">
                ...
                <Badge className={`text-sm px-4 py-1.5 ${isAllCompleted ? 'bg-green-100 dark:bg-green-950/20 text-green-700 dark:text-green-400' : 'bg-blue-50 dark:bg-blue-950/20 text-blue-600 dark:text-blue-400'}`}>
                  ...
                  {challenges.map((task) => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-6 rounded-2xl border transition-all text-left ${
                        task.done 
                          ? 'bg-zinc-50/50 dark:bg-zinc-100/30 border-zinc-200 dark:border-zinc-200 cursor-default' 
                          : 'bg-white dark:bg-zinc-50 border-zinc-200 dark:border-zinc-200 shadow-sm hover:border-blue-300 dark:hover:border-blue-500 hover:scale-[1.01] active:scale-[0.99]'
                      }`}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${task.done ? 'bg-green-100 dark:bg-green-950/30 text-green-600 dark:text-green-400' : 'bg-zinc-100 dark:bg-zinc-150 text-zinc-400 dark:text-zinc-500'}`}>
                          {task.done ? <CheckCircle2 size={20} /> : <div className="w-3 h-3 rounded-full bg-zinc-350 dark:bg-zinc-200"/>}
                        </div>
            ...
            <div className="bg-white dark:bg-zinc-50 p-10 rounded-[2rem] border border-zinc-100 dark:border-zinc-200 shadow-sm transition-colors duration-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                {user.badges && user.badges.map(b => {
                  ...
                  return (
                    <div key={b.id} className={`flex flex-col items-center text-center p-6 rounded-2xl ${b.bg} dark:bg-zinc-150/40 border border-white/50 dark:border-zinc-200 shadow-sm hover:-translate-y-1 transition-transform cursor-pointer`}>
                      <div className={`w-14 h-14 rounded-full bg-white dark:bg-zinc-50 flex items-center justify-center mb-4 shadow-sm ${b.color}`}>
                        <BadgeIcon size={28} />
                      </div>
                ...
                <div className="flex flex-col items-center justify-center text-center p-6 rounded-2xl border-2 border-dashed border-zinc-200 dark:border-zinc-200 text-zinc-400 dark:text-zinc-500">
                  <div className="w-14 h-14 rounded-full bg-zinc-50 dark:bg-zinc-150 flex items-center justify-center mb-4"><Trophy size={24} /></div>
