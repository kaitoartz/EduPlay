  ...
  if (status === 'finished') {
    return (
      <div className="min-h-screen flex items-center justify-center px-6 pt-20 pb-10 bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white transition-colors duration-300">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: "spring", damping: 20 }} className="max-w-lg w-full mx-auto p-12 bg-white dark:bg-zinc-50 rounded-[3rem] border border-zinc-100 dark:border-zinc-200 shadow-2xl dark:shadow-none text-center relative overflow-hidden">
    ...
    return (
      <div className="min-h-screen flex flex-col items-center pt-28 pb-10 px-4 md:px-8 bg-zinc-50/50 dark:bg-zinc-950 text-zinc-900 dark:text-white transition-colors duration-300">
        <div className="w-full max-w-6xl flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-zinc-50 p-6 rounded-[2rem] border border-zinc-150 dark:border-zinc-200 shadow-sm">
    ...
    <div className="min-h-screen flex flex-col items-center pt-32 pb-20 px-6 bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white transition-colors duration-300">
      <div className="w-full max-w-3xl">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={currentQ} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="bg-white dark:bg-zinc-550/10 p-10 md:p-14 rounded-[3rem] shadow-xl dark:shadow-none shadow-zinc-200/50 border border-zinc-100 dark:border-zinc-200">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {q.options.map((opt, i) => {
                ...
                let btnClass = "bg-white dark:bg-zinc-50 border-2 border-zinc-200 dark:border-zinc-200 text-zinc-700 dark:text-zinc-300 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-950/20";
                if (status !== 'idle') {
                  ...
                  else btnClass = "bg-zinc-50 dark:bg-zinc-150 border-2 border-zinc-200 dark:border-zinc-200 text-zinc-400 dark:text-zinc-650 opacity-50";
                }
                return (
                  <button key={i} onClick={() => handleSelect(i)} disabled={status !== 'idle'} className={`relative p-8 rounded-[2rem] font-bold text-2xl transition-all duration-300 transform active:scale-95 ${btnClass}`}>
                    {opt}
                    {status !== 'idle' && isCorrect && <motion.div initial={{scale:0}} animate={{scale:1}} className="absolute -top-4 -right-4 bg-white dark:bg-zinc-50 rounded-full text-green-550 shadow-lg"><CheckCircle2 size={36} fill="currentColor" className="text-white bg-green-500 rounded-full"/></motion.div>}
                    {status !== 'idle' && isSelected && !isCorrect && <motion.div initial={{scale:0}} animate={{scale:1}} className="absolute -top-4 -right-4 bg-white dark:bg-zinc-50 rounded-full text-red-500 shadow-lg"><XCircle size={36} fill="currentColor" className="text-white bg-red-500 rounded-full"/></motion.div>}
                  </button>
                )
              })}
            </div>
